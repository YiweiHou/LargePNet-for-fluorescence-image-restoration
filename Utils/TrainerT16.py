...
def Val_step(val_data, model, criterion2, criterion3):
    ...
    with torch.no_grad():
        for i in range(len(val_data)):
            b_x, b_y = val_data[i]
            b_x = b_x.to(device)  # 将输入数据移动到GPU上
            b_y = b_y.to(device)  # 将目标数据移动到GPU上
            b_x = b_x.unsqueeze(0).unsqueeze(2)
            b_y = b_y.unsqueeze(0)
        ...
        MSE_val = MSE_val_epoch / len(val_data)
        MAE_val = MAE_val_epoch / len(val_data)
        #SSIM_val = SSIM_val_epoch / len(val_data)
        PSNR_val = PSNR_val_epoch / len(val_data)
        val_loss = MSE_val
    return MSE_val, MAE_val, 0, PSNR_val, val_loss


def TrainerT16(model, train_data, val_data, save_path, opt):
    ...
    for epoch in range(epoch_num):
        ...
        for step, (b_x, b_y) in enumerate(train_loader):
            ...
            b, n_frames, h_lr, w_lr = b_x.size()
            idx_center = (n_frames - 1) // 2

            b_x, b_y = Variable(b_x), Variable(b_y)
            b_x = b_x.unsqueeze(2)
            #b_y = b_y.view(b, -1, 1, h_lr * 2, w_lr * 2)
