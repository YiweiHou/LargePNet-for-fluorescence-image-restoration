...
def DataAug(raw, gt, aimingsize):
    ...
    out_raw, out_gt = random_cropXY(raw, gt, aimingsize)
    values = [1, 2, 3]
    random_numbers = np.random.choice(values, size=1)
...
def DataAugKeep(raw, gt):
    # Assue raw and gt are torch tensor
    raw = raw.squeeze(0).squeeze(0)
    ...
    values = [1, 2, 3]
    random_numbers = np.random.choice(values, size=1)
    out_raw, out_gt = flip_XY(raw, gt, random_numbers)

    random_numbers = np.random.choice(values, size=1)
    out_raw, out_gt = rotate_XY(out_raw, out_gt, random_numbers)
    out_raw = out_raw.copy()
    out_gt = out_gt.copy()
    out_raw = torch.tensor(out_raw).unsqueeze(0).unsqueeze(0)
    out_gt = torch.tensor(out_gt).unsqueeze(0).unsqueeze(0)
    return out_raw, out_gt